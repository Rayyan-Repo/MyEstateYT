...
<title>Search Properties — MyEstate</title>
...
/* SEARCH FORM */
.search-form-wrap{background:var(--white);border-radius:2rem;padding:3rem;border:1.5px solid var(--line);box-shadow:0 16px 56px rgba(214,40,40,.1);margin-top:2rem;}
.sf-title{font-family:'Cormorant Garamond',serif;font-size:2.4rem;font-weight:700;color:var(--ink);margin-bottom:2rem;}
.sf-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(20rem,1fr));gap:1.4rem;margin-bottom:2rem;}
...
.sf-btn{display:inline-flex;align-items:center;gap:.7rem;background:linear-gradient(135deg,var(--r),var(--rd));color:#fff;border:none;border-radius:1.2rem;padding:1.2rem 3rem;font-size:1.3rem;font-weight:700;cursor:pointer;font-family:'Outfit',sans-serif;box-shadow:0 6px 20px rgba(214,40,40,.35);transition:all .25s;}
.sf-btn:hover{transform:translateY(-2px);box-shadow:0 12px 32px rgba(214,40,40,.48);}
...
@media(max-width:1100px){.foot-grid{grid-template-columns:1fr 1fr;gap:3rem;}.listings-grid{grid-template-columns:repeat(auto-fill,minmax(32rem,1fr));}}
...
<section class="page-hero">
  <div class="page-hero-inner">
    <div class="eyebrow">Search Properties</div>
    <h1 class="page-title">Find Your <em>Dream Home</em></h1>
  </div>
</section>
...
<section class="listings-sec">
  <div class="search-form-wrap">
    <div class="sf-title"><i class="fas fa-filter" style="color:var(--r);margin-right:.8rem;"></i>Filter Your Search</div>
    <form action="" method="post">
      <div class="sf-grid">
        <div class="sf"><label>Location</label><input type="text" name="location" placeholder="City or area name..." value="<?= isset($_POST['location']) ? htmlspecialchars($_POST['location']) : ''; ?>"></div>
        <div class="sf"><label>Property Type</label>
          <select name="type">
            <option value="">Any type</option>
            <option value="flat" <?= (isset($_POST['type']) && $_POST['type']=='flat') ? 'selected' : ''; ?>>Apartment / Flat</option>
            <option value="house" <?= (isset($_POST['type']) && $_POST['type']=='house') ? 'selected' : ''; ?>>House / Villa</option>
            <option value="shop" <?= (isset($_POST['type']) && $_POST['type']=='shop') ? 'selected' : ''; ?>>Commercial / Shop</option>
          </select>
        </div>
        <div class="sf"><label>Offer Type</label>
          <select name="offer">
        ...
        <div class="sf"><label>BHK</label>
          <select name="bhk">
            <option value="">Any BHK</option>
            <?php for($i=1;$i<=9;$i++): ?>
            <option value="<?= $i; ?>" <?= (isset($_POST['bhk']) && $_POST['bhk']==$i) ? 'selected' : ''; ?>><?= $i; ?> BHK</option>
            <?php endfor; ?>
          </select>
        </div>
        <div class="sf"><label>Min Budget (₹)</label>
          <select name="min">
            <option value="0">No min</option>
            <option value="500000" <?= (isset($_POST['min']) && $_POST['min']=='500000') ? 'selected' : ''; ?>>5 Lakh</option>
            <option value="1000000" <?= (isset($_POST['min']) && $_POST['min']=='1000000') ? 'selected' : ''; ?>>10 Lakh</option>
            <option value="2000000" <?= (isset($_POST['min']) && $_POST['min']=='2000000') ? 'selected' : ''; ?>>20 Lakh</option>
            <option value="3000000" <?= (isset($_POST['min']) && $_POST['min']=='3000000') ? 'selected' : ''; ?>>30 Lakh</option>
            <option value="5000000" <?= (isset($_POST['min']) && $_POST['min']=='5000000') ? 'selected' : ''; ?>>50 Lakh</option>
            <option value="10000000" <?= (isset($_POST['min']) && $_POST['min']=='10000000') ? 'selected' : ''; ?>>1 Cr</option>
            <option value="20000000" <?= (isset($_POST['min']) && $_POST['min']=='20000000') ? 'selected' : ''; ?>>2 Cr</option>
            <option value="50000000" <?= (isset($_POST['min']) && $_POST['min']=='50000000') ? 'selected' : ''; ?>>5 Cr</option>
          </select>
        </div>
        <div class="sf"><label>Max Budget (₹)</label>
          <select name="max">
            <option value="999999999">No max</option>
            <option value="1000000" <?= (isset($_POST['max']) && $_POST['max']=='1000000') ? 'selected' : ''; ?>>10 Lakh</option>
            <option value="2000000" <?= (isset($_POST['max']) && $_POST['max']=='2000000') ? 'selected' : ''; ?>>20 Lakh</option>
            <option value="3000000" <?= (isset($_POST['max']) && $_POST['max']=='3000000') ? 'selected' : ''; ?>>30 Lakh</option>
            <option value="5000000" <?= (isset($_POST['max']) && $_POST['max']=='5000000') ? 'selected' : ''; ?>>50 Lakh</option>
            <option value="10000000" <?= (isset($_POST['max']) && $_POST['max']=='10000000') ? 'selected' : ''; ?>>1 Cr</option>
            <option value="20000000" <?= (isset($_POST['max']) && $_POST['max']=='20000000') ? 'selected' : ''; ?>>2 Cr</option>
            <option value="50000000" <?= (isset($_POST['max']) && $_POST['max']=='50000000') ? 'selected' : ''; ?>>5 Cr</option>
            <option value="100000000" <?= (isset($_POST['max']) && $_POST['max']=='100000000') ? 'selected' : ''; ?>>10 Cr</option>
          </select>
        </div>
        <div class="sf"><label>Status</label>
          <select name="status">
            <option value="">Any status</option>
            <option value="ready to move" <?= (isset($_POST['status']) && $_POST['status']=='ready to move') ? 'selected' : ''; ?>>Ready to Move</option>
            <option value="under construction" <?= (isset($_POST['status']) && $_POST['status']=='under construction') ? 'selected' : ''; ?>>Under Construction</option>
          </select>
        </div>
        <div class="sf"><label>Furnished</label>
          <select name="furnished">
            <option value="">Any</option>
            <option value="furnished" <?= (isset($_POST['furnished']) && $_POST['furnished']=='furnished') ? 'selected' : ''; ?>>Furnished</option>
            <option value="semi-furnished" <?= (isset($_POST['furnished']) && $_POST['furnished']=='semi-furnished') ? 'selected' : ''; ?>>Semi-Furnished</option>
            <option value="unfurnished" <?= (isset($_POST['furnished']) && $_POST['furnished']=='unfurnished') ? 'selected' : ''; ?>>Unfurnished</option>
          </select>
        </div>
      </div>
      <div style="display:flex;gap:1rem;align-items:center;flex-wrap:wrap;">
        <button type="submit" name="filter_search" class="sf-btn"><i class="fas fa-search"></i> Search Properties</button>
        <?php if(isset($_POST['filter_search'])): ?>
        <a href="search.php" class="sf-btn" style="background:var(--white);color:var(--r);border:1.5px solid rgba(214,40,40,.2);box-shadow:none;"><i class="fas fa-times"></i> Clear Filters</a>
        <?php endif; ?>
      </div>
    </form>
  </div>
  ...
  if(isset($_POST['filter_search'])){
     ...
     if(!empty($_POST['location'])){
        ...
        $search_params[] = '%' . $_POST['location'] . '%';
     }
     if(!empty($_POST['type'])){
        $search_where[] = "type LIKE ?";
        $search_params[] = '%' . $_POST['type'] . '%';
     }
     if(!empty($_POST['offer'])){
        $search_where[] = "offer LIKE ?";
        $search_params[] = '%' . $_POST['offer'] . '%';
     }
     if(!empty($_POST['bhk'])){
        $search_where[] = "bhk = ?";
        $search_params[] = $_POST['bhk'];
     }
     if(!empty($_POST['status'])){
        $search_where[] = "status LIKE ?";
        $search_params[] = '%' . $_POST['status'] . '%';
     }
     if(!empty($_POST['furnished'])){
        $search_where[] = "furnished LIKE ?";
        $search_params[] = '%' . $_POST['furnished'] . '%';
     }
     if(!empty($_POST['min'])){
        $search_where[] = "price >= ?";
        $search_params[] = $_POST['min'];
     }
     if(!empty($_POST['max']) && $_POST['max'] != '999999999'){
        $search_where[] = "price <= ?";
        $search_params[] = $_POST['max'];
     }
  ...
  <div style="margin-top:3rem;margin-bottom:1rem;">
    <?php if(isset($_POST['filter_search'])): ?>
    <h2 style="font-family:'Cormorant Garamond',serif;font-size:2.8rem;font-weight:700;color:var(--ink);">Search Results <span style="font-size:1.8rem;color:var(--ink3);font-weight:400;">(<?= $select_properties->rowCount(); ?> found)</span></h2>
    <?php else: ?>
    <h2 style="font-family:'Cormorant Garamond',serif;font-size:2.8rem;font-weight:700;color:var(--ink);">Latest <span style="color:var(--r);font-style:italic;">Listings</span></h2>
    <?php endif; ?>
  </div>
